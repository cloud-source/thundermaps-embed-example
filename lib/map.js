...
function createMap(opts) {
    ...
    const goToReportOnClick = opts.goToReportOnClick;
    ...
    const getFieldData = memoize(async (channel) => {
        const options = {
            uri: api + "channels/"+channel+"/form",
            headers: headers()
        };
        ...
        return new Map(data.fields.map(i => [i.key, i.label]));
    });
    const makeMarker = memoize((item) => {
        ...
        if (!goToReportOnClick) {
            ...
            var listener = img.addEventListener("click", async ()=> {
                img.removeEventListener("click", listener);
                const options = {
                    uri: api + "reports/"+item.id,
                    qs: {fields: "form_fields,user_short_name,account_name"},
                    headers: headers()
                };
                let data = JSON.parse((await request('get', options.uri, options)).body);
                const fieldMap = await(getFieldData(data.account_id));
                let fieldData = "";
                let fields = data.form_fields;
                console.log(fields);
                if (config.visible_fields) {
                    fields = fields.filter(field => config.visible_fields.indexOf(field.key) !== -1);
                }

                fields.forEach(field => {
                        fieldData += `
                        <h5>${fieldMap.get(field.key)}</h5>
                        <p>${field.value}</p>
                    `;
                    });
                popup.setHTML(`
